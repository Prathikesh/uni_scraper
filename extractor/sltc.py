...
class SLTCExtractor(BaseExtractor):
    ...
    def extract(self, html, url):
        ...
        # ==================================
        # COURSE NAME (SLTC — FINAL & CORRECT)
        # ==================================

        course_name = None

        # 1️⃣ Correct container (from real DOM)
        h2 = soup.select_one(
        ...
        # 2️⃣ Fallback: any valid Elementor h2 (ignore slogans / CTAs)
        if not course_name:
        ...
        # 3️⃣ LAST fallback: derive from URL slug (never use site slogan)
        if not course_name:
            course_name = (
                url.rstrip("/")
                .split("/")[-1]
