...
export default function DiplomaForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/recommend/diploma", formData);
  ...
  const fieldCount = formData.diploma_field ? 1 : 0;
  const gpaCount = formData.gpa && formData.gpa > 0 ? 1 : 0;
  const checkboxCount = [formData.institution_recognized, formData.english].filter(Boolean).length;
  const progressPercent = (fieldCount * 25) + (gpaCount * 25) + (checkboxCount * 25);
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <Link 
              to="/select-level" 
              className="inline-flex items-center space-x-2 text-purple-700 hover:text-purple-800 font-medium group"
            >
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-xl border border-purple-100">
                <div className="bg-gradient-to-br from-purple-500 to-violet-600 w-16 h-16 rounded-2xl flex items-center justify-center mb-4 shadow-lg">
                ...
                <p className="text-gray-600">
                  Enter your diploma details to discover suitable courses
                </p>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-purple-100">
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div>
                    ...
                    <select
                      value={formData.diploma_field}
                      onChange={(e) => setFormData({ ...formData, diploma_field: e.target.value })}
                      className="w-full px-4 py-4 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all text-lg font-medium"
                      required
                    >
                    ...
                    <p className="mt-2 text-sm text-gray-500">
                      Select your diploma field of study
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-2 text-sm text-gray-500">
                      Enter your diploma GPA on a 4.0 scale. Higher GPA increases match scores.
                    </p>
                  </div>
                    ...
                    <div className="space-y-3">
                      <motion.label 
                        ...
                        className={`relative flex items-center p-5 border-2 rounded-xl cursor-pointer transition-all ${
                          formData.institution_recognized 
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={formData.institution_recognized}
                          onChange={(e) => setFormData({ ...formData, institution_recognized: e.target.checked })}
                          className="sr-only"
                        />
                        <div className="flex items-center space-x-3">
                          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                            ...
                            {formData.institution_recognized && <CheckCircle2 className="w-4 h-4 text-white" />}
                          </div>
                          <span className="font-medium text-gray-900">Institution is Recognized</span>
                        </div>
                      </motion.label>
                  ...
                  <motion.button
                    ...
                    type="submit"
                    disabled={loading}
                    className={`w-full bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white font-bold py-5 rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl ${
